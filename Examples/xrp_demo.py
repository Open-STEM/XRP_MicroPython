...
def balance():
    ...
    while time.time() < start_time+20:
        kpPitch = 1/30 # set so that robot will go max speed at 20 degrees
        kpAngle = 0.0000025 # Readings in millidegrees, expecting a low kp
        gyroReadings = xrp.imu.get_gyro()
        error = kpAngle * gyroReadings[2]
        effort = kpPitch * pitch()
        print(f"Effort: {effort}, Error: {error}")
        xrp.drivetrain.set_effort(effort + error, effort - error)
        time.sleep(0.05)
    xrp.drivetrain.stop()
...
def ramp_demo():
    ...
    while True:
        ...
        drive_until_change(direction*0.65, direction*10, 2)
        # Balance on ramp
        balance()
